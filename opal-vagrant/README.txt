...
Our current Vagrant/Puppet setup and Opal packaging is targeting a .deb based Linux distribution (Debian or Ubuntu).
...
1 - Install Vagrant from http://www.vagrantup.com/downloads

2 - Download the latest Opal Vagrant bundle from url
        https://repo.thehyve.nl/service/local/artifact/maven/redirect?r=snapshots&g=org.obiba.opal&a=opal-vagrant&e=zip&v=LATEST&c=vagrant
    This is a very small file that only contains descriptors on how to prepare the guest VM and required dependencies and system tweaks
...
3 - Unzip this file to a folder in the host machine (where you want your Vagrant image to be located)
    ...
    If you need to customize something, create a 'config.yaml' copying from 'config.yaml.sample',
    edit the values you want and comment/remove all the others.
	...
	Examples of what you can modify in the config.yaml:
		...
		-aspects of the VM hardware, like number of CPUs, memory, etc..
...
Please refer to Vagrant documentation (http://docs.vagrantup.com/v2/getting-started/index.html) for more information.
...
The easiest is to create the databases in the local running instance of MySQL.
